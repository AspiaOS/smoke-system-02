...
export const Route = createFileRoute("/control/contas_/$id")({
  ...
  notFoundComponent: () => <ControlShell title="Conta"><div className="text-neutral-400">Conta não encontrada.</div></ControlShell>,
...
function ContaDetalhePage() {
  ...
  return (
    <ControlShell title={data.display_name || data.email || "Conta"}>
      <div className="mb-6">
        <Link to="/control/contas" className="text-xs text-neutral-500 hover:text-neutral-300">← Voltar</Link>
      </div>
      ...
      <section className="border border-neutral-800 rounded-lg p-5 bg-[#111014] mb-6">
        ...
        <dl className="grid grid-cols-2 gap-3 text-sm">
          <div><dt className="text-neutral-500 text-xs">Email</dt><dd>{data.email || "—"}</dd></div>
          <div><dt className="text-neutral-500 text-xs">Status</dt><dd>{data.status}</dd></div>
          <div><dt className="text-neutral-500 text-xs">Papel plataforma</dt><dd>{data.platform_role ?? "—"}</dd></div>
          <div><dt className="text-neutral-500 text-xs">Criada em</dt><dd>{new Date(data.created_at).toLocaleString("pt-BR")}</dd></div>
          <div><dt className="text-neutral-500 text-xs">Último acesso</dt><dd>{data.last_seen_at ? new Date(data.last_seen_at).toLocaleString("pt-BR") : "—"}</dd></div>
        </dl>
      ...
      <section className="border border-neutral-800 rounded-lg p-5 bg-[#111014] mb-6">
        <h2 className="text-sm uppercase tracking-wide text-neutral-500 mb-3">Memberships</h2>
        {data.memberships.length === 0 ? (
          <p className="text-sm text-neutral-500">Sem vínculos.</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.memberships.map((m) => (
                <tr key={m.store_id} className="border-t border-neutral-800">
                  <td className="py-2">
                    <Link to="/control/lojas/$id" params={{ id: m.store_id }} className="text-violet-300 hover:underline">
                      {m.store_name}
                    </Link>
                  </td>
                  <td>
        ...
        {canChangeRole && (
          <div className="mt-4 flex gap-2 items-end border-t border-neutral-800 pt-4">
            <div className="flex-1">
              <label className="text-xs text-neutral-500">Adicionar vínculo</label>
            ...
            <div>
              <label className="text-xs text-neutral-500">Papel</label>
      ...
      {data.invitations.length > 0 && (
        <section className="border border-neutral-800 rounded-lg p-5 bg-[#111014] mb-6">
          <h2 className="text-sm uppercase tracking-wide text-neutral-500 mb-3">Convites</h2>
          <table className="w-full text-sm">
            <thead className="text-neutral-500 text-xs uppercase">
            ...
            <tbody>
              {data.invitations.map((i) => (
                <tr key={i.id} className="border-t border-neutral-800">
                  ...
                  <td>{i.role}</td>
                  <td className="text-neutral-400">{i.status}</td>
      ...
      {data.recent_events.length > 0 && (
        <section className="border border-neutral-800 rounded-lg p-5 bg-[#111014]">
          <h2 className="text-sm uppercase tracking-wide text-neutral-500 mb-3">Eventos recentes</h2>
          <ul className="space-y-2 text-sm">
            {data.recent_events.map((e) => (
              <li key={e.id} className="border-t border-neutral-800 pt-2">
                <div className="flex justify-between">
                  <span className="text-neutral-200">{e.action}</span>
                  <span className="text-xs text-neutral-500">{new Date(e.created_at).toLocaleString("pt-BR")}</span>
                </div>
                {e.payload_json && <pre className="text-xs text-neutral-500 mt-1 overflow-x-auto">{e.payload_json}</pre>}
              </li>
            ))}
          </ul>
        </section>
