...
BEGIN
  ...
  SELECT store_id, name, delivery_fee
    INTO v_store_id, v_neighborhood_name, v_delivery_fee
    FROM public.neighborhoods
   WHERE id = p_neighborhood_id AND active = true;
  IF v_store_id IS NULL THEN
    RAISE EXCEPTION 'Bairro indisponível';
  END IF;

  -- Upsert customer by (store, phone)
  INSERT INTO public.customers (store_id, phone, name, last_address, last_neighborhood)
  ...
  SELECT whatsapp_number INTO v_whatsapp
    FROM public.store_settings WHERE store_id = v_store_id;

  RETURN QUERY SELECT v_order_id, v_subtotal, v_delivery_fee, v_total, COALESCE(v_whatsapp, '');
END;
$$;
